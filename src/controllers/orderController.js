...
export const createOrder = async (req, res) => {
  try {
    ...
    // Cambio automático a 'completed' después de 1 minuto si se proporcionó dirección de envío
    if (shippingAddress) {
      setTimeout(async () => {
        try {
          ...
          if (orderToUpdate && orderToUpdate.status === "shipped") {
            ...
            console.log(`⏱️ Orden ${orderToUpdate._id} cambió automáticamente a estado: completed`);
          }
        } catch (err) {
          console.error("Error al actualizar orden a completed automáticamente:", err);
        }
      }, 1 * 60 * 1000); // 1 minuto
    }
...
export const updateOrderStatus = async (req, res) => {
  try {
    ...
    console.log(`✏️ Estado de la orden ${order._id} actualizado manualmente a: ${status}`);
